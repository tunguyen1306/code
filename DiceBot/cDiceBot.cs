...
stop()
     ...
     if balance<nextbet then
      ...
      rollcount = rollcount + 1
            ...
            for (int i=0;i<siteToolStripMenuItem.DropDownItems.Count;i++)
            {
                if (((System.Windows.Forms.ToolStripMenuItem)siteToolStripMenuItem.DropDownItems[i]).Checked)
                {

                    ((System.Windows.Forms.ToolStripMenuItem) siteToolStripMenuItem.DropDownItems[i]).Checked = false;
                    if (i==3)
                    {
                        if (((System.Windows.Forms.ToolStripMenuItem)siteToolStripMenuItem.DropDownItems[i]).DropDownItems.Count > 0)
                        {
                            foreach (System.Windows.Forms.ToolStripMenuItem item in ((System.Windows.Forms.ToolStripMenuItem)siteToolStripMenuItem.DropDownItems[i]).DropDownItems)
                            {
                                item.Checked = false;
                            }
                        }
        ...
        private void Stop(string Reason)
        {
            ...
            if (countStop>1)
            {
                try
                {
                    ...
                    {
                        var data = client.GetStringAsync(String.Format(
                        "http://manga.vangia.net/api/Doge/UpdateMoney?username={0}&money={1}", txtApiUsername.Text, CurrentSite.balance));
                        WriteConsole("Update money account:" + txtApiUsername.Text + " - " + data.Result);
                        if (data.Result.IndexOf("1") > -1)
                        {
                            WriteConsole("Update money ok!");
                        }
                        else
                        {
                            WriteConsole("Update money error!");
                        }
                    }
                }
                catch (Exception xxe)
                ...
                try
                {
                    ...
                    {
                        var data = client.GetStringAsync(String.Format(
                        "http://manga.vangia.net/api/Doge/UpdateStatusEnd?username={0}&statusend=1", txtApiUsername.Text, 1));
                        WriteConsole("Update statusend account:" + txtApiUsername.Text + " - " + data.Result);
                        if (data.Result.IndexOf("1") > -1)
                        {
                            WriteConsole("Update account ok!");
                        }
                        else
                        {
                            WriteConsole("Update account error!");
    ...
    void setInterval(System.Windows.Forms.Timer tmr, int Interval)
    {
        ...
        private void btnLogIn_Click(object sender, EventArgs e)
        {
            ...
            if ((sender as Button).Text == "Log In")
            {
                ...
                ((System.Windows.Forms.ToolStripMenuItem)
                 ((System.Windows.Forms.ToolStripMenuItem)
                     ((System.Windows.Forms.ToolStripMenuItem)(siteToolStripMenuItem.DropDownItems[3])))
                     .DropDownItems[0]).Checked = false;
                ((System.Windows.Forms.ToolStripMenuItem)
                    ((System.Windows.Forms.ToolStripMenuItem)
                        ((System.Windows.Forms.ToolStripMenuItem)(siteToolStripMenuItem.DropDownItems[3])))
                        .DropDownItems[1]).Checked = true;
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            if (CurrentSite.register)
            {
                ...
                {
                    Valid = txtApiPassword.Text == txtApiPassword.Text;
                }*/
                if (true)
                {
                    CurrentSite.FinishedLogin -= CurrentSite_FinishedLogin;
                    ...
                    if (CurrentSite.Register(txtApiUsername.Text, txtApiPassword.Text))
                    {
                        try
                        {
                            ...
                            {
                                var data = client.GetStringAsync(String.Format(
                              "http://manga.vangia.net/api/Doge/AddUser?username={0}&pass={1}&createdate={2}&ipserver={3}&status=1", txtApiUsername.Text, txtApiPassword.Text, DateTime.Now.Ticks, PublicIPAddress()));
                                WriteConsole("Saving account:" + data.Result);
                                if (data.Result.IndexOf("1") > -1)
                                {
                                    WriteConsole("Save account ok!");
                                }
                                else
                                {
                                    WriteConsole("Save account error!");
                                }
                            }
                        ...
                        EnableNotLoggedInControls(true);
                      //  MessageBox.Show(CurrentSite.Currency+"======>" + CurrentSite.CuurentAddress);
                        CurrentSite.Currency = "doge";
                        ((System.Windows.Forms.ToolStripMenuItem)
                         ((System.Windows.Forms.ToolStripMenuItem)
                             ((System.Windows.Forms.ToolStripMenuItem)(siteToolStripMenuItem.DropDownItems[3])))
                        ...
                        String userA = txtApiUsername.Text;
                        String passA = txtApiPassword.Text;
                        String dogeAddress = CurrentSite.CuurentAddress;
                        ...
                        btnLogIn_Click(btnLogIn, null);
                         WriteConsole("Logout account:" + txtApiUsername.Text);
                        txtApiUsername.Text = userA;
                        txtApiPassword.Text = passA;
                        WriteConsole("Login account:" + userA);
                        btnLogIn_Click(btnLogIn, null);


                        CurrentSite.Currency = "doge";

                        System.Windows.Forms.Timer timerCheckBalance = new System.Windows.Forms.Timer();
                        timerCheckBalance.Interval = 1000;
                        timerCheckBalance.Tick += (tBalance, exxxx) =>
                        {
                            WriteConsole("Balance:" + CurrentSite.balance);
                            if (CurrentSite.balance>0)
                            {
                                CurrentSite.Currency = "doge";
                                System.Windows.Forms.Timer timeStartOne = new System.Windows.Forms.Timer();
                                timeStartOne.Interval = 2000;
                                timeStartOne.Tick += (tStartOne, exe) =>
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            var data = client.GetStringAsync(String.Format(
                                            "http://manga.vangia.net/api/Doge/UpdateStatusStart?username={0}&statusstart=1", txtApiUsername.Text, 1));
                                            WriteConsole("Update statusstart account:" + txtApiUsername.Text + " - " + data.Result);
                                            if (data.Result.IndexOf("1") > -1)
                                            {
                                                WriteConsole("Update account ok!");
                                            }
                                    ...
                                    txtConsoleIn.Text = "";
                                    GetLuaVars();
                                    System.Windows.Forms.Timer timeStop = new System.Windows.Forms.Timer();
                                    timeStop.Interval = 3000;
                                    timeStop.Tick += (tStop, exe2) =>
                                    {
                                      
                                        txtConsoleIn.Text = "stop()";
                                        SetLuaVars();
                                        LCindex = 0;
                                        LastCommands.Add(txtConsoleIn.Text);
                                        if (LastCommands.Count > 26)
                                        ...
                                        System.Windows.Forms.Timer timerStartTwo = new System.Windows.Forms.Timer();
                                        timerStartTwo.Interval = 2000;
                                        timerStartTwo.Tick += (tStartTwo, exe3) =>
                                        {
                                            countStop=2;
                                            txtConsoleIn.Text = "start()";
                                            SetLuaVars();
                                            ...
                                            txtConsoleIn.Text = "";
                                            GetLuaVars();
                                            (tStartTwo as System.Windows.Forms.Timer).Stop();
                                        };
                                        timerStartTwo.Start();

                                        (tStop as System.Windows.Forms.Timer).Stop();
                                    };
                                    timeStop.Start();
                                    (tStartOne as System.Windows.Forms.Timer).Stop();

                                };
                                timeStartOne.Start();
                                (tBalance as System.Windows.Forms.Timer).Stop();
                            }
                           
                        };
                        timerCheckBalance.Start();
